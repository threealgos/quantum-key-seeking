...
def main():
    ...
    # Even Faster Post-Processing: Vectorized GCD/filter, minimal fraction trials, pre-dedup bitstrings
    # Efficiency: Numpy for batch GCD; single fraction limit; skip ZNE full runs if SEARCH_DEPTH small
    import numpy as np  # For vectorized operations

    counts_list = [counts]
    if config.USE_ZNE and config.SEARCH_DEPTH > 1000:  # Skip ZNE for small depths to save time
        logger.info("Running optimized scaled circuits for ZNE...")
        for scale in [3, 5]:
            scaled_shots = config.SHOTS // scale
            if scaled_shots < 1024: scaled_shots = 1024  # Minimum shots threshold
            scaled_counts = run_kernel(kernel_func, config, dxs, dys, scaled_shots)
            counts_list.append(scaled_counts)
    final_counts = manual_zne(counts_list) if config.USE_ZNE else counts

    # Pre-filter and dedup top bitstrings (list of unique with count > min_threshold)
    min_count_threshold = max(1, config.SHOTS // 1000)  # Dynamic threshold to skip noise
    top_bitstrings = [bitstr for bitstr, cnt in final_counts.most_common(config.SEARCH_DEPTH) if cnt >= min_count_threshold]

    measurements_set = set()
    den_limit = ORDER  # Single limit for speed (use //2 if too slow, but single is faster)
    ...
    for bitstr in top_bitstrings:
        ...
        # MSB: Reverse + same (batch to avoid redundant int conversions)
        val_msb = int(bitstr[::-1], 2)
    ...
    # Vectorized filter: Convert to numpy array for batch GCD (faster for large sets)
    measurements_arr = np.array(list(measurements_set))
    gcd_arr = np.gcd(measurements_arr, ORDER)
    filtered = measurements_arr[(measurements_arr != 0) & (gcd_arr == 1)].tolist()

    # BB correction (on filtered list, already deduped)
    ...
    print("="*60)
    found = False
    top_candidates = sorted(filtered, reverse=True)[:15]
    ...
    # Viz (optimized: downsample if too many entries)
    if len(final_counts) > 100:  # Downsample for plot speed
        downsampled_counts = dict(final_counts.most_common(100))
    else:
        downsampled_counts = final_counts

    plt.figure(figsize=(12, 6))
    plt.subplot(2, 1, 1)
    plt.bar(range(len(downsampled_counts)), list(downsampled_counts.values()))
    plt.title("Distribution (Top Entries)")
    plt.xticks(range(0, len(downsampled_counts), max(1, len(downsampled_counts)//10)),
               [hex(int(k, 2))[:10] for k in list(downsampled_counts)[::max(1, len(downsampled_counts)//10)]], rotation=45)
